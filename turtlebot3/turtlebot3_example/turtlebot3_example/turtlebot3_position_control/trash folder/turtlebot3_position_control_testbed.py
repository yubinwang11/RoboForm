...
class Turtlebot3PositionControl(Node):
    ...
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.B_safed=0.08
        self.buttonstart=0
        self.detection_r=20*self.axis
        self.source_position_xs=0.5
        self.source_position_ys=0.5
        ...
        self.distance_state_cur=np.array([0.0 for x in range(self.node_num)])

        self.distance0_cur=np.array([0.0 for x in range(self.node_num)])
        self.distance1_cur=np.array([0.0 for x in range(self.node_num)])
        self.distance2_cur=np.array([0.0 for x in range(self.node_num)])
        self.distance6_cur=np.array([0.0 for x in range(self.node_num)])

        self.distance0_pre=np.array([0.0 for x in range(self.node_num)])
        self.distance1_pre=np.array([0.0 for x in range(self.node_num)])
        self.distance2_pre=np.array([0.0 for x in range(self.node_num)])
        self.distance6_pre=np.array([0.0 for x in range(self.node_num)])

        self.SVE=np.array([0.0 for x in range(self.node_num)])
        self.SPE_x=np.array([0.0 for x in range(self.node_num)])       #source_position_estimate_x
        self.SPE_y=np.array([0.0 for x in range(self.node_num)])       #source_position_estimate_y
        self.SPEG_x=np.array([0.0 for x in range(self.node_num)])      #source_position_estimate_gradient_x
        self.SPEG_y=np.array([0.0 for x in range(self.node_num)])      #source_position_estimate_gradient_y
        
        self.SPC_x=np.array([0.0 for x in range(self.node_num)])       #source_position_consensus_x
        self.SPC_y=np.array([0.0 for x in range(self.node_num)])       #source_position_consensus_y
        self.SPCP_x=np.array([0.0 for x in range(self.node_num)])       #source_position_consensus_pre_x
        self.SPCP_y=np.array([0.0 for x in range(self.node_num)])       #source_position_consensus_pre_y
    ...
    def explore_localization(self):
        ...
        if self.buttonstart==1:
            for i in range (0,self.node_num,1):
                self.distance1_cur[i]=(0.00*random.randint(0,9)+1)*math.sqrt((self.source_position_xs-self.x_cur[i])**2+(self.source_position_ys-self.y_cur[i])**2)
            ...
            for i in range (0,self.node_num,1):
                ...
                self.distance1_pre[i]=self.distance1_cur[i]   
                self.SPCP_x[i]=self.SPC_x[i]
                self.SPCP_y[i]=self.SPC_y[i]     
    ...
    def init_system(self): 
        ...
        if self.position_init==1:
            ...
            for i in range (0,self.node_num,1):
                ...
                self.SPC_x[i]=self.x_cur[i]+self.distance1_cur[i]*math.cos(self.d_cur[i])
                self.SPC_y[i]=self.y_cur[i]+self.distance1_cur[i]*math.sin(self.d_cur[i])
...
##########################################################################################################################          
    def unicycle_simulation(self):
        ...
        for i in range (0,self.node_num,1):
            if self.distance1_cur[i]>self.detection_r:
                roo[i]=0    
            else:
                roo[i]=self.distance1_cur[i]
        ...
        for i in range(0,self.node_num,1):
            ...
            #plt.plot(self.x_o_body[i],self.y_o_body[i],self.colorArr[i],linestyle=':')  #检测距离R
            
            
            #plt.plot(self.SPC_x[i],self.SPC_y[i],self.colorArr[i],marker='*')
        ...
        distance=math.sqrt((sumx/self.node_num-self.source_position_xs)**2+(sumy/self.node_num-self.source_position_ys)**2)
